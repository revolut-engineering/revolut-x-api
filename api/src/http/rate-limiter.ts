...
export class RateLimiter {
  ...
  private readonly publicEndpoints: TokenBucket;

  constructor(generalRpm: number = 1000, publicPer10s: number = 20) {
    this.general = new TokenBucket(generalRpm, 60);
    this.publicEndpoints = new TokenBucket(publicPer10s, 10);
  }
  ...
  async acquirePublic(): Promise<void> {
    await this.publicEndpoints.acquire();
  }
}
