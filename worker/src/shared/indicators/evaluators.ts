/**
 * Alert type dispatch — evaluates alert conditions against market data.
 */
...
function evalRsi(
  ...
  const met =
    direction === "above" ? rsi.gte(threshold) : rsi.lte(threshold);

  const emoji = rsi.gte(70) ? "\uD83D\uDD34" : rsi.lte(30) ? "\uD83D\uDFE2" : "\uD83D\uDFE1";
  return {
    conditionMet: met,
...
function evalMacd(
  ...
  const met =
    direction === "bullish"
      ? macdLine.gt(signalLine)
      : macdLine.lt(signalLine);
...
function evalObi(
  ...
  const met =
    direction === "above" ? obi.gte(threshold) : obi.lte(threshold);
