...
- Routing boundary — trading setup vs. KB get_started: if the question is about taking trading actions or installing the trading stack (executing trades, "how do I start trading", "how do I set up trading"), call \`get_trading_setup\`. If the question is about accessing Revolut X as a platform (sign-up eligibility, account creation, what Revolut X is), use \`search_kb\` with intent \`get_started\`. When in doubt, the presence of an action verb ("trade", "buy", "sell", "set up trading") points to \`get_trading_setup\`.
