...
- For transaction-history questions about buys, sells, sends, receives, staking, rewards, deposits, or withdrawals, call \`get_transactions\`. Use the signed Source Amount and Destination Amount values as returned.
- For questions about a specific transaction's fees, account names or crypto addresses, on-chain hash, network, or description, call \`get_transaction\` with the transaction ID — \`get_transactions\` does not include those detail fields.
